...
def load_sample_images():
    ...
    # 1. Underexposed (terlalu gelap)
    dark = cv2.imread("gelap.jpg", cv2.IMREAD_GRAYSCALE)
    dark = cv2.resize(dark,(size,size))
    
    # 2. Overexposed (terlalu terang)
    bright = cv2.imread("terang.jpg", cv2.IMREAD_GRAYSCALE)
    bright = cv2.resize(bright,(size,size))

    # 3. Uneven Illumination (Bayangan/silau)
    uneven = cv2.imread("tidakMerata.jpg", cv2.IMREAD_GRAYSCALE)
    uneven = cv2.resize(uneven,(size,size))

    # 4. Low contrast synthetic image
    low_contrast = np.random.normal(loc=128, scale=20, size=(size, size))
...
images['dark'], images['bright'], images['uneven']
...
for name, img in images.items():
    negative_result = negative_transform(img)
    log_result = log_transform(img, c=50)
    gamma_05 = power_law_transform(img, gamma=0.5)
    gamma_10 = power_law_transform(img, gamma=1.0)
    gamma_20 = power_law_transform(img, gamma=2.0)

# Visualize transformations
fig, axes = plt.subplots(2, 4, figsize=(16, 8))

# Original
axes[0, 0].imshow(img, cmap='gray', aspect='auto')
...
plt.tight_layout()
plt.show()
# 2. HISTOGRAM ANALYSIS - BASED ENCHANCEMENT 
print("\n2. HISTOGRAM ANALYSIS - BASED ENCHANCEMENT")
...
low_contrast_img = sample_images['dark']
